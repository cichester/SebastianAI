  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 pb-20">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2 mb-6 tracking-tight">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm text-slate-600 mb-2">Lingua</label>
            <select
              value={language}
          ...
          <div>
            <label className="block text-sm text-slate-600 mb-2">Livello CEFR</label>
             <select
              value={level}
          ...
          <div>
            <label className="block text-sm text-slate-600 mb-2">Numero di File (Varianti)</label>
          ...
          <div>
            <label className="block text-sm text-slate-600 mb-2">Argomento Principale</label>
            <input
              type="text"
              value={topic.name}
              onChange={(e) => setTopic(prev => ({ ...prev, name: e.target.value }))}
              placeholder="es. Present Perfect vs Past Simple, Vocabulary: Travel..."
              className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-slate-900 bg-white"
              required
            />
          </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
         <div className="flex justify-between items-center mb-6">
            ...
            <span className="bg-slate-100 text-slate-600 py-1 px-3 rounded-full text-xs font-semibold">Totale: {totalExercises} items</span>
         </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
           {mainExTypes.map(type => {
             ...
             return (
               <div key={type.key} className={`border rounded-lg p-5 transition-colors ${isSelected ? 'border-emerald-500 bg-emerald-50/20' : 'border-slate-200'}`}>
                 <div className="flex flex-col h-full">
                    <div className="flex items-start justify-between mb-4">
                       <div className={`p-2 rounded-md ${isSelected ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>
                    ...
                    <div className="font-bold text-slate-800 mb-6">{type.label}</div>
                    
                    <div className="mt-auto flex items-center justify-between border border-slate-200 rounded-md bg-white">
                      <button 
                        type="button" 
                        onClick={() => handleExerciseChange(type.key, count - 1)}
                        className="px-3 py-2 text-slate-500 hover:text-slate-800 hover:bg-slate-50 rounded-l-md"
                        disabled={!isSelected}
                      >&minus;</button>
                      <input 
                        type="number" 
                        ...
                        className="w-12 text-center text-sm font-semibold border-none focus:ring-0 text-slate-800 p-0"
                        disabled={!isSelected}
                      />
                      <button 
                        type="button" 
                        onClick={() => handleExerciseChange(type.key, count + 1)}
                        className="px-3 py-2 text-slate-500 hover:text-slate-800 hover:bg-slate-50 rounded-r-md"
                        disabled={!isSelected}
                      >&#43;</button>
                    </div>
                 </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
        ...
        <div className="space-y-4">
          {/* Reading */}
          <label className={`block border rounded-lg p-4 cursor-pointer transition-colors ${topic.reading.enabled ? 'border-emerald-500 bg-emerald-50/20' : 'border-slate-200 hover:border-slate-300'}`}>
             <div className="flex items-start gap-4">
               ...
               <div>
                  <div className="flex items-center gap-2">
                     ...
                     <span className="font-bold text-slate-800">Aggiungi Lettura (Reading Comprehension)</span>
                  </div>
                  <p className="text-sm text-slate-500 mt-1">Genera un testo originale con 5 domande di comprensione associate.</p>
               </div>
             </div>
          </label>

          {/* Listening */}
           <label className={`block border rounded-lg p-4 cursor-pointer transition-colors ${topic.listening.enabled ? 'border-emerald-500 bg-emerald-50/20' : 'border-slate-200 hover:border-slate-300'}`}>
             <div className="flex items-start gap-4">
               <div className="pt-0.5">
                  <input 
                    type="checkbox" 
               ...
               <div>
                  <div className="flex items-center gap-2">
                     ...
                     <span className="font-bold text-slate-800">Aggiungi Ascolto (Listening Script)</span>
                  </div>
                  <p className="text-sm text-slate-500 mt-1">Genera uno script per l'insegnante con domande di ascolto collegate.</p>
               </div>
             </div>
          </label>

          {/* Writing */}
           <label className={`block border rounded-lg p-4 cursor-pointer transition-colors ${topic.writing.enabled ? 'border-emerald-500 bg-emerald-50/20' : 'border-slate-200 hover:border-slate-300'}`}>
             <div className="flex items-start gap-4">
               <div className="pt-0.5">
                  <input 
                    type="checkbox" 
               ...
               <div>
                  <div className="flex items-center gap-2">
                     ...
                     <span className="font-bold text-slate-800">Aggiungi Scrittura (Writing Task)</span>
                  </div>
                  <p className="text-sm text-slate-500 mt-1">Includi un prompt di produzione scritta basato sull'argomento principale.</p>
               </div>
             </div>
          </label>
        </div>
      ...
      <div className="fixed bottom-0 left-64 right-0 p-6 bg-slate-50/80 backdrop-blur-sm border-t border-slate-200 flex justify-between items-center z-10 print:hidden">
         <span className="text-sm text-slate-600 font-medium">Pronto per generare le varianti?</span>
         <div className="flex gap-4">
            <button 
              ...
              className="px-4 py-2.5 border border-slate-300 rounded-lg font-medium text-slate-600 bg-white hover:bg-slate-50 transition-colors flex items-center gap-2"
            ...
            <button 
              ...
              className="px-4 py-2.5 border border-slate-300 rounded-lg font-medium text-slate-700 bg-white hover:bg-slate-50 transition-colors flex items-center gap-2"
            ...
            <button 
              ...
              className={`px-6 py-2.5 rounded-lg font-bold text-white flex items-center gap-2 transition-colors ${
                isFormValid() && !isLoading ? 'bg-emerald-500 hover:bg-emerald-600 shadow-sm shadow-emerald-500/20' : 'bg-slate-300 text-slate-500 cursor-not-allowed'
              }`}
              title={!isFormValid() ? "Inserisci un argomento e seleziona almeno un esercizio per continuare" : "Genera il quiz con i dati inseriti"}
            >
               <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5">
