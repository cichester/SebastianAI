...
const Sidebar: React.FC<SidebarProps> = ({ user, currentView, setCurrentView, onLogout, onOpenSettings, onOpenSupport, theme, toggleTheme }) => {
  return (
    <aside className="fixed inset-y-0 left-0 w-64 bg-[#1e293b] dark:bg-[#0f172a] text-white flex flex-col justify-between print:hidden transition-colors border-r border-slate-800/50">
        ...
        <nav className="flex flex-col gap-2 px-4">
          <button
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${
              currentView === 'dashboard' ? 'bg-[#047857] text-white shadow-sm shadow-emerald-900/20' : 'text-slate-300 hover:bg-slate-800/80'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${
              currentView === 'create' ? 'bg-[#047857] text-white shadow-sm shadow-emerald-900/20' : 'text-slate-300 hover:bg-slate-800/80'
            }`}
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
          ...
          <button
            ...
            className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors text-slate-300 hover:bg-slate-800/80"
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
      ...
      <div className="p-4 flex flex-col gap-2 border-t border-slate-800/50 mt-auto">
        <button 
          onClick={toggleTheme}
          className="flex items-center justify-between w-full gap-3 px-4 py-3 rounded-lg text-sm font-medium text-slate-300 hover:bg-slate-800/80 transition-colors"
        >
          <div className="flex items-center gap-3">
            {theme === 'dark' ? <SunIcon className="w-5 h-5 text-amber-400" /> : <MoonIcon className="w-5 h-5 text-blue-300" />}
            <span>{theme === 'dark' ? 'Modalità Chiara' : 'Modalità Scura'}</span>
          </div>
          <div className={`w-10 h-5 rounded-full relative transition-colors duration-200 ${theme === 'dark' ? 'bg-emerald-600' : 'bg-slate-600'}`}>
            <div className={`absolute top-1 bg-white w-3 h-3 rounded-full shadow transition-all duration-200 ${theme === 'dark' ? 'right-1' : 'left-1'}`} />
          </div>
        </button>

        <button 
          onClick={onOpenSupport}
          className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-slate-300 hover:bg-slate-800/80 transition-colors"
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
        ...
        <button 
          onClick={onLogout}
          className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-rose-400 hover:bg-rose-500/10 transition-colors mt-2"
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
